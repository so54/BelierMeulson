...
<nav class="navbar fixed-top navbar-expand-lg bg-blue">
    <div  id="navbarSupportedContent" >
        <ul class="nav navbar-nav mr-auto" >
           <li class="<?php active2("/galerie"); ?>"><a class="navbar-brand hover " href="{{ route('galerie') }}">Photos</a></li>
            <li class="dropdown {{currentRoute(route('profile.edit', auth()->id()))}} ">
                <a   href="#" class="dropdown-toggle hover" data-toggle="dropdown" >Profil</a>
                <ul class="dropdown-menu ">
                    <li class="{{currentRoute(route('profile.edit', auth()->id()))}}"><a href="{{ route('profile.edit', auth()->id()) }}">  <i class="fa fa-id-card "></i> Modifier mon profil</a></li>
    ...
    @admin        <li class="dropdown {{currentRoute(route('evenement.create'))}} <?php active2("/evenement") ;active2("/photo/create");  active2("/maintenance"); active2("/user"); active2("evenement/edit"); active2('/register');?>">
                ...
                <ul class="dropdown-menu ">
                    ...
                    <li class="<?php  active2("/user");active2("/register"); ?>"> <a  href="{{ route('user.index') }}"><i class="fa fa-users "></i> Utilisateurs</a></li>
                </ul>
            </li>@endadmin
       ...
       <select onchange="document.location.href=this.value" class="selectnav " id="selectnav2">
          ...
          <option <?php select2("/galerie"); ?> value="{{route('galerie')}}"  >Photos</option>
           ...
           <option  {{currentSelect(route('profile.edit', auth()->id()))}} value=" {{route('profile.edit', auth()->id())}}">→ Modifier mon profil</option>
           ...
           @admin   <option value="#">Administration</option>
            ...
            <option <?php select2("/evenement"); ?> value="{{route('evenement.index')}}">→ Gérer les albums</option>
           ...
           <option <?php  select2("/user");select2("/register"); ?> value="{{ route('user.index') }}">→ Utilisateurs</option>
